    ...
    ui(new Ui::DlgItem)
{
    ...
    on_combo2_currentTextChanged(QStringLiteral("华东"));
    if(mItem)
    {
        //修改
	...
	connect(ui->combo2,SIGNAL(currentTextChanged(QString)),this,SLOT(on_combo2_currentTextChanged(const QString)));
}
...
void DlgItem::on_btnAdd_clicked()
{
    ...
    else
    {
        ...
        ui->edit1->setFocus();
        ui->labelTip->setText(QString::fromLocal8Bit("增加成功!"));
    }
...
{
    ui->combo3->clear();
    if(text == QStringLiteral("华东"))
    {
        ui->combo3->addItems(QStringList()<< QStringLiteral("山东") << QStringLiteral("浙江") << QStringLiteral("江苏") << QStringLiteral("安徽") << QStringLiteral("上海") << QStringLiteral("福建"));
    }
    else if(text == QStringLiteral("华南"))
    {
        ui->combo3->addItems(QStringList()<< QStringLiteral("广东") << QStringLiteral("广西") << QStringLiteral("海南"));
	}
    else if(text == QStringLiteral("华中"))
    {
        ui->combo3->addItems(QStringList()<< QStringLiteral("湖北") << QStringLiteral("湖南") << QStringLiteral("河南") << QStringLiteral("江西"));
	}
    else if(text == QStringLiteral("华北"))
    {
        ui->combo3->addItems(QStringList()<< QStringLiteral("北京") << QStringLiteral("天津") << QStringLiteral("河北") << QStringLiteral("山西") << QStringLiteral("内蒙"));
	}
    else if(text == QStringLiteral("其他"))
    {
        ui->combo3->addItems(QStringList()<< QStringLiteral("宁夏") << QStringLiteral("新疆") << QStringLiteral("青海") << QStringLiteral("陕西") << QStringLiteral("甘肃") << QStringLiteral("四川") << QStringLiteral("云南") << QStringLiteral("贵州") << QStringLiteral("西藏") << QStringLiteral("重庆") << QStringLiteral("辽宁") << QStringLiteral("吉林") << QStringLiteral("黑龙江"));
	}
}
